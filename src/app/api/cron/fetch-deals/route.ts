 ...
 * - 6 Norwegian airports: OSL, TRF, TRD, BGO, SVG, TOS
 * - 153 destinations from Flajts sitemap = 918 routes
 * - 3× daily cron (06:00, 12:00, 18:00 Norwegian time = 05:00, 11:00, 17:00 UTC)
 * - Each run processes 1/3 of destinations (51 dests × 6 airports = 306 routes)
 * - 2 API calls per route (OW + RT) = 612 calls per run
 * - 3 runs × 612 = 1,836 calls/day = ~55,080 calls/month (well within 150k budget)
 * - Every destination gets refreshed once per day across the 3 runs
 *
 * Deal detection: price ≥25% below average across returned dates
 * Stores best deal per route (one-way or round-trip, whichever has bigger discount)
 */

// EXACT 153 destinations from Flajts sitemap.xml
const DESTINATIONS = [
...
// All 6 Norwegian departure airports (TRF = Torp/Sandefjord)
...
/**
 * Returns the start date for getPriceGraph: first day of next month.
 * This ensures we always look at future travel dates.
 */
...
export async function GET(request: Request) {
  ...
  // Split 153 destinations into 3 equal batches (one per daily run)
  // Run index determined by current UTC hour:
  //   05:00 UTC → batch 0 (dests 0-50)
  //   11:00 UTC → batch 1 (dests 51-101)
  //   17:00 UTC → batch 2 (dests 102-152)
  const BATCH_COUNT = 3
  const BATCH_SIZE = Math.ceil(DESTINATIONS.length / BATCH_COUNT)
  const utcHour = new Date().getUTCHours()
  let runIndex: number
  if (utcHour >= 14) {
    runIndex = 2  // 17:00 UTC run
  } else if (utcHour >= 8) {
    runIndex = 1  // 11:00 UTC run
  } else {
    runIndex = 0  // 05:00 UTC run
  }
  ...
  for (const origin of ORIGINS) {
    for (const dest of destBatch) {
      ...
      try {
        ...
        // Pick best deal type (higher discount wins)
        ...
        dealsToUpsert.push({
          ...
          typical_price_low: ow?.price ?? null,    // best one-way price
          typical_price_high: rt?.price ?? null,   // best round-trip price
  ...
  if (dealsToUpsert.length > 0) {
    ...
    // Also store raw price history for future deal-detection algorithm
  ...
  return NextResponse.json({
    ...
    startDate,
    returnDate,
    runBatch: `${runIndex + 1}/${BATCH_COUNT} (dests ${runIndex * BATCH_SIZE + 1}–${Math.min((runIndex + 1) * BATCH_SIZE, DESTINATIONS.length)} of ${DESTINATIONS.length})`,
    origins: ORIGINS.length,
    routesInBatch: destBatch.length * ORIGINS.length,
