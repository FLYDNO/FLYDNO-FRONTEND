        ...
        <main style={{ flex: 1, overflowY: 'auto', background: '#050505' }}>
          ...
          <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '32px 24px' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginBottom: '32px' }}>
              {airports.map((a) => (
                <button key={a} onClick={() => setFilter(a)} style={{ padding: '8px 16px', borderRadius: '100px', fontSize: '13px', fontWeight: filter === a ? 700 : 500, background: filter === a ? '#ff6b00' : '#111', border: filter === a ? 'none' : '1px solid #1e1e1e', color: filter === a ? '#fff' : 'rgba(255,255,255,0.4)', cursor: 'pointer', transition: 'all 0.2s', fontFamily: "'DM Sans', sans-serif" }}>{a}</button>
              ))}
            </div>
