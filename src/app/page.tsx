...
export default function HomePage() {
  ...
  function buildTicker() {
    ...
    const DESTS = [
      {f:'es',n:'Barcelona'},{f:'gb',n:'London'},{f:'th',n:'Bangkok'},{f:'us',n:'New York'},{f:'jp',n:'Tokyo'},
      {f:'it',n:'Roma'},{f:'ae',n:'Dubai'},{f:'fr',n:'Paris'},{f:'de',n:'Berlin'},{f:'gr',n:'Aten'},
      {f:'pt',n:'Lisboa'},{f:'id',n:'Bali'},{f:'sg',n:'Singapore'},{f:'ma',n:'Marrakech'},{f:'tr',n:'Istanbul'},
      {f:'ca',n:'Toronto'},{f:'au',n:'Sydney'},{f:'is',n:'Reykjavik'},{f:'za',n:'Cape Town'},{f:'nl',n:'Amsterdam'},
      {f:'cz',n:'Praha'},{f:'hr',n:'Dubrovnik'},{f:'mx',n:'Mexico City'},{f:'in',n:'Mumbai'},{f:'eg',n:'Kairo'},
      {f:'kr',n:'Seoul'},{f:'mv',n:'Maldivene'},{f:'ee',n:'Tallinn'},{f:'co',n:'Bogotá'},{f:'br',n:'Rio de Janeiro'},
    ]
    ...
    <>
      ...
      <style>{`
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        .nav-links a{color:rgba(255,255,255,0.6);transition:color .2s;font-weight:500}
        .nav-links a:hover{color:#fff}
        .nav-btn{padding:8px 18px;background:#ff6b00;color:#fff;border-radius:100px;font-size:13px;font-weight:700;border:none;cursor:pointer;transition:background .2s}
        ...
        .btn-primary{display:inline-flex;align-items:center;gap:6px;padding:13px 26px;background:#ff6b00;color:#fff;border-radius:100px;font-size:14px;font-weight:700;border:none;cursor:pointer;transition:all .2s}
        .btn-primary:hover{background:#e55f00;transform:translateY(-1px)}
        .btn-outline{display:inline-flex;align-items:center;gap:6px;padding:13px 22px;background:transparent;color:rgba(255,255,255,0.8);border-radius:100px;font-size:14px;font-weight:500;border:1px solid rgba(255,255,255,0.12);cursor:pointer;transition:all .2s}
        ...
        .deal-card{background:#242424;border:1px solid rgba(255,255,255,0.10);border-radius:14px;padding:20px;transition:border-color .25s,transform .25s,box-shadow .25s,background .25s}
        ...
        .deal-card-book{display:inline-flex;align-items:center;gap:5px;font-size:12px;font-weight:600;color:rgba(255,255,255,0.4);margin-top:12px;padding:6px 0;border-bottom:1px solid rgba(255,255,255,0.08);width:100%;transition:color .2s}
        ...
        .how-card{background:#242424;border:1px solid rgba(255,255,255,0.10);border-radius:14px;padding:28px;transition:border-color .25s,transform .25s,background .25s}
        ...
        .feat-card{background:#242424;border:1px solid rgba(255,255,255,0.10);border-radius:14px;padding:24px;transition:border-color .25s,transform .25s,background .25s}
        ...
        .price-btn{display:block;width:100%;padding:15px;border-radius:100px;font-family:'DM Sans',sans-serif;font-size:15px;font-weight:700;text-align:center;cursor:pointer;border:none;background:#ff6b00;color:#fff;transition:all .2s}
        ...
        .faq-btn{width:100%;display:flex;align-items:center;justify-content:space-between;padding:22px 0;background:none;border:none;color:#f0f0f0;font-family:'DM Sans',sans-serif;font-size:15px;font-weight:700;cursor:pointer;text-align:left;gap:16px;transition:color .2s}
        .faq-btn:hover{color:#ff6b00}
        .faq-icon{font-size:18px;color:rgba(255,255,255,0.4);transition:transform .3s;flex-shrink:0}
        .faq-item.open .faq-icon{transform:rotate(45deg)}
        .faq-ans{max-height:0;overflow:hidden;transition:max-height .3s ease,padding .3s ease}
        ...
        .t-dest{display:inline-flex;align-items:center;gap:6px;padding:5px 14px;border-radius:100px;font-size:13px;font-weight:500;color:rgba(255,255,255,0.55);border:1px solid transparent;white-space:nowrap;transition:color .18s,border-color .18s,background .18s;cursor:default}
      ...
      <nav className="navbar">
        <div className="nav-inner">
          <Link href="/" className="nav-logo">
            <div className="nav-logo-icon"><span className="ms" style={{fontSize:'16px',color:'#fff'}}>flight_takeoff</span></div>
            FlyDeals
          </Link>
      ...
      <section className="hero">
        <div className="container">
          <div className="hero-grid">
            <div>
              <div className="hero-badge"><span className="dot"></span><span>{dealCount}+ deals funnet</span></div>
              ...
              <div className="hero-btns">
                <Link href="/login" className="btn-primary">Prøv gratis i dag <svg width="14" height="14" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><path d="M3 7h8M8 4l3 3-3 3"/></svg></Link>
                <a href="#how" className="btn-outline">Se hvordan det fungerer</a>
              </div>
              <div className="hero-trust">
                {['7 dager gratis','Ingen binding','Fri til å stoppe'].map(t=>(
                  <span key={t} className="trust-item"><svg width="13" height="13" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><path d="M2 6.5l3.5 3.5 5.5-6"/></svg>{t}</span>
                ))}
              </div>
            </div>
            <div className="hero-right">
              <div className="deal-preview-card">
                ...
                {[{from:'fra Oslo (OSL)',dest:'New York',flag:'us',price:'2 890 kr',unit:'t/r',disc:'-41%'},{from:'fra Bergen (BGO)',dest:'London',flag:'gb',price:'489 kr',unit:'enkel',disc:'-52%'},{from:'fra Oslo (OSL)',dest:'Bangkok',flag:'th',price:'2 489 kr',unit:'t/r',disc:'-47%'}].map((d,i)=>(
                  <div key={i} className="deal-row">
                    <div><div className="deal-from">{d.from}</div><div className="deal-dest">{d.dest} <span className={`fi fi-${d.flag} fis`} style={{width:'1.1em',height:'0.85em',display:'inline-block',verticalAlign:'middle',marginBottom:'1px',borderRadius:'2px'}}></span></div></div>
                    <div className="deal-price-col"><div className="deal-price-val">{d.price}</div><div className="deal-price-unit">{d.unit}</div><div className="deal-badge">{d.disc}</div></div>
                  </div>
                ))}
              </div>
              <div className="hero-map-hint"><span className="ms" style={{fontSize:'16px',color:'#ff6b00'}}>public</span>Overvåker 237+ destinasjoner fra 6 norske flyplasser</div>
            </div>
          </div>
        </div>
      </section>

      <div className="ticker-wrap"><div className="ticker-track" id="ticker"></div></div>
      ...
      <section style={{position:'relative',height:'620px',overflow:'hidden',margin:0}}>
        ...
        <div style={{position:'absolute',inset:0,display:'flex',alignItems:'center',justifyContent:'center'}}>
          <div style={{textAlign:'center',maxWidth:'620px',padding:'0 24px'}}>
            <div style={{display:'inline-flex',alignItems:'center',gap:'8px',background:'rgba(255,107,0,0.15)',border:'1px solid rgba(255,107,0,0.35)',borderRadius:'100px',padding:'6px 16px',fontSize:'12px',fontWeight:600,color:'#ff6b00',letterSpacing:'0.08em',textTransform:'uppercase' as const,marginBottom:'20px'}}>
              <span style={{width:'6px',height:'6px',background:'#ff6b00',borderRadius:'50%',display:'inline-block'}}></span>Global dekning
            </div>
            ...
            <div style={{display:'flex',alignItems:'center',justifyContent:'center',gap:'12px',flexWrap:'wrap' as const}}>
              {['Overvåker kontinuerlig','✅ 6 norske flyplasser','🌍 237+ destinasjoner'].map((t,i)=>(
                <div key={i} style={{display:'inline-flex',alignItems:'center',gap:'8px',background:'rgba(5,5,5,0.82)',border:'1px solid rgba(255,255,255,0.1)',borderRadius:'100px',padding:'10px 18px',fontFamily:"'DM Sans',sans-serif",fontSize:'13px',color:'rgba(255,255,255,0.85)',backdropFilter:'blur(12px)'}}>
                  {i===0&&<span style={{width:'7px',height:'7px',background:'#22c55e',borderRadius:'50%',boxShadow:'0 0 8px #22c55e',animation:'pulse 2s infinite',display:'inline-block'}}></span>}{t}
                </div>
              ))}
            </div>
          </div>
      ...
      <section className="deals-strip">
        <div className="container">
          ...
          <div className="deals-cards">
            {[{iata:'OSL',airport:'Oslo Gardermoen',dest:'Firenze',flag:'it',price:'849 kr',unit:'t/r',disc:'-38%',delay:'0s'},{iata:'BGO',airport:'Bergen Flesland',dest:'Barcelona',flag:'es',price:'520 kr',unit:'t/r',disc:'-44%',delay:'.08s'},{iata:'SVG',airport:'Stavanger Sola',dest:'London',flag:'gb',price:'3 190 kr',unit:'t/r',disc:'-31%',delay:'.16s'}].map((d,i)=>(
              <div key={i} className="deal-card fade-up" style={{transitionDelay:d.delay}}>
                ...
                <div className="deal-card-footer"><div><div className="deal-card-price">{d.price} <span>{d.unit}</span></div></div><div className="deal-card-discount">{d.disc}</div></div>
          ...
          <div style={{textAlign:'center',marginTop:'28px'}}><Link href="/deals" className="btn-outline" style={{display:'inline-flex'}}>Se alle deals →</Link></div>
        </div>
      </section>
      ...
      <section className="section" id="how" style={{borderTop:'1px solid rgba(255,255,255,0.05)'}}>
        <div className="container">
          ...
          <div className="how-grid">
            <div className="how-card fade-up"><div className="how-step">Steg 01</div><div className="how-icon"><span className="ms" style={{fontSize:'18px'}}>schedule</span></div><h3>Vi overvåker prisene</h3><p>Algoritmene våre sjekker flypriser kontinuerlig og bygger opp et bilde av hva som er normalt – og hva som er tilbud.</p></div>
            <div className="how-card fade-up" style={{transitionDelay:'.09s'}}><div className="how-step">Steg 02</div><div className="how-icon"><span className="ms" style={{fontSize:'18px'}}>mail</span></div><h3>Du får et varsel</h3><p>Når vi finner en pris som er vesentlig lavere enn normalt, sender vi deg et varsel direkte på e-post.</p><div className="how-mini"><div className="how-mini-row"><span style={{fontSize:'11px',color:'rgba(255,255,255,0.5)'}}>OSL → Bangkok</span><span className="how-mini-price">2 489 kr</span></div><div className="how-mini-row"><span style={{fontSize:'11px',color:'rgba(255,255,255,0.5)'}}>BGO → London</span><span className="how-mini-price">489 kr</span></div><div className="how-mini-row" style={{border:'none',paddingTop:'8px'}}><span className="how-mini-badge">-47% under snitt</span></div></div></div>
            <div className="how-card fade-up" style={{transitionDelay:'.18s'}}><div className="how-step">Steg 03</div><div className="how-icon"><span className="ms" style={{fontSize:'18px'}}>check_circle</span></div><h3>Du booker og sparer</h3><p>Book direkte hos flyselskapet til den lave prisen. Ingen mellomledd, ingen ekstra kostnader.</p></div>
          </div>
        </div>
      </section>
      ...
      <section className="section" style={{borderTop:'1px solid rgba(255,255,255,0.05)'}}>
        <div className="container">
          <div style={{textAlign:'center',maxWidth:'600px',margin:'0 auto'}}><h2 className="section-title">Alt du trenger for<br/>billigere flyreiser</h2></div>
          <div className="features-grid">
            {[{icon:'price_check',title:'Transparent prising',desc:'Vi viser alltid prisen mot det historiske snittet, slik at du kan se nøyaktig hvor mye du sparer.',delay:'0s'},{icon:'autorenew',title:'Automatisk overvåking',desc:'Prisene sjekkes automatisk slik at du aldri trenger å gjøre det manuelt.',delay:'.06s'},{icon:'open_in_new',title:'Direkte booking',desc:'Vi sender deg direkte til flyselskapet. Ingen mellomledd eller ekstra kostnader.',delay:'.12s'},{icon:'public',title:'Nordisk dekning',desc:'Vi overvåker de største flyplassene i Norden for å finne de beste prisene.',delay:'.18s'},{icon:'dashboard',title:'Enkel oversikt',desc:'På ditt dashboard er det enkelt å finne den neste flyvningen til en nesten umulig pris.',delay:'.24s'},{icon:'savings',title:'Spar tid og penger',desc:'Vi gjør prisovervåkingen, du nyter reisen til en brøkdel av ordinærprisen.',delay:'.30s'}].map((f,i)=>(
              <div key={i} className="feat-card fade-up" style={{transitionDelay:f.delay}}><div className="feat-icon"><span className="ms" style={{fontSize:'17px'}}>{f.icon}</span></div><h3>{f.title}</h3><p>{f.desc}</p></div>
            ))}
          </div>
        </div>
      </section>
      ...
      <section className="pricing" id="pricing">
        <div className="container">
          <div style={{textAlign:'center'}}><h2 className="section-title">Invester i billigere reiser</h2><p className="section-sub">Spar inn hele årsmedlemskapet på din første bestilling.</p></div>
          <div className="pricing-inner">
            <div className="pricing-card">
              ...
              <ul className="price-features">
                {['Alle deals – ingen begrensninger','Sanntids e-postvarsler','6+ norske flyplasser overvåket','Ingen bindingstid'].map(f=>(
                  <li key={f}><svg width="14" height="14" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><path d="M2 7l4 4 6-6"/></svg>{f}</li>
                ))}
              </ul>
      ...
      <section className="faq" id="om">
        <div className="container">
          <div style={{textAlign:'center'}}><div className="section-tag" style={{justifyContent:'center'}}><span className="dot"></span> Spørsmål & svar</div><h2 className="section-title">Ofte stilte spørsmål</h2></div>
          <div className="faq-list">
            {faqs.map((f,i)=>(
              <div key={i} className={`faq-item${openFaq===i?' open':''}`}>
                <button className="faq-btn" onClick={()=>setOpenFaq(openFaq===i?null:i)}>{f.q}<span className="ms faq-icon">add</span></button>
        ...
        <div className="container">
          <div className="footer-grid">
            <div className="footer-brand">
              <div className="nav-logo" style={{display:'inline-flex',marginBottom:'8px'}}><div className="nav-logo-icon"><span className="ms" style={{fontSize:'14px',color:'#fff'}}>flight_takeoff</span></div><span style={{fontSize:'15px',fontWeight:800}}>FlyDeals</span></div>
            ...
            <div className="footer-col"><h4>Produkt</h4><Link href="/deals">Live deals</Link><Link href="/oppdag">Oppdag ruter</Link><Link href="/varsler">Dine varsler</Link><a href="#">FAQ</a></div>
            <div className="footer-col"><h4>Selskap</h4><a href="#">Personvern</a><a href="#">Vilkår</a><Link href="/brukerstotte">Hjelp</Link></div>
            <div className="footer-col"><h4>Følg oss</h4><a href="#">Twitter / X</a><a href="#">Instagram</a></div>
          </div>
          <div className="footer-bottom"><span>© 2026 FlyDeals. Alle rettigheter reservert.</span></div>
        </div>
      </footer>
    </>
  )
